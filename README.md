...
docker run --rm -v "$PWD/tests:/app/tests" ghcr.io/mikkeljuhl/vouch:0.3.2

# Emit JUnit to the host:
docker run --rm \
  -v "$PWD/tests:/app/tests" \
  -v "$PWD/reports:/app/reports" \
  ghcr.io/mikkeljuhl/vouch:0.3.2 --reporter=junit --reporter-outfile=/app/reports/junit.xml
```
...
jobs:
  test:
    ...
    steps:
      ...
      - uses: mikkeljuhl/vouch@v0.3.2    # pin a release tag (or @main for latest)
        with:
...
(`@v0.3.2`) for stability, or track `@main` for the latest.
...
MIT (c) Mikkel Juhl. See [`LICENSE`](./LICENSE).
